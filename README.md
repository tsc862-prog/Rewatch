...
Every past fight row can carry a quiet grey number after the stars: the "other users" score, which is Verdict MMA's fan rating for that bout (`crowd_rating` 0–10 and `crowd_rating_count` on `fight_search`, scraped nightly by the scraper repo's `verdict_scraper.py`) halved to the 0–5 star scale so it reads against the stars beside it. It is shown as-is and is not blended with the user's own rating; the tooltip gives the raw score and the number of fans. UFC bonus awards (`bonus_awards`: `FOTN`, `POTN`, `KOTN`, `SOTN`) are each a tag (same family as TITLE BOUT, slightly larger for legibility) pushed to the right end of the sub-meta line that carries the weight class and event name, with the full name in the tooltip. Both the number and the tags sit behind the row's spoiler gate: while the result is hidden ("Rate to reveal result") nothing is shown at all, since a crowd score reveals whether the fight was any good and a bonus name reveals a finish; they appear with the result once the fight is rated. `crowdScore()`, `crowdScoreHtml()` and `bonusTagsHtml()` in `js/events.js` hold this. Tables and view columns were added on September 16, 2026 (scraper repo `migrate/add_crowd_ratings.sql`).
