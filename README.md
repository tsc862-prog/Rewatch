...
Run `node tests/rating-saves.cjs` for delayed-save, overlapping-note, retry, and account-switch regression checks, and `node tests/crowd-blend.cjs` for the crowd score and bonus tags.

## Crowd score

Every past fight row can carry a quiet grey number after the stars: the **other users' average rating** for that bout on the 0–5 star scale. The app has no real user base yet, so that average is currently sourced from Verdict MMA's fans (`crowd_rating` 0–10 and `crowd_rating_count` on `fight_search`, scraped nightly by the scraper repo's `verdict_scraper.py`), halved to stars. It is shown as-is and is never blended with the viewer's own rating; the tooltip says where it comes from, with the raw score and the number of fans. When real users arrive, `crowdScore()` should read an RPC that averages other users' ratings (excluding the viewer) with the crowd; nothing else in the row changes. UFC bonus awards (`bonus_awards`: `FOTN`, `POTN`, `KOTN`, `SOTN`) are each a tag (same family as TITLE BOUT, slightly larger for legibility) pushed to the right end of the sub-meta line that carries the weight class and event name, with the full name in the tooltip. Both the number and the tags sit behind the row's spoiler gate: while the result is hidden ("Rate to reveal result") nothing is shown at all, since a crowd score reveals whether the fight was any good and a bonus name reveals a finish; they appear with the result once the fight is rated. `crowdScore()`, `crowdScoreHtml()` and `bonusTagsHtml()` in `js/events.js` hold this. Tables and view columns were added on September 16, 2026 (scraper repo `migrate/add_crowd_ratings.sql`).

The Community tab uses the same crowd ratings as its "other users": while the app has no real user base, `community_dashboard()` reads each fight's average and count from `fight_crowd_ratings` (halved to stars) instead of the `ratings` table, so the community average, the highest-rated fights (20+ ratings), cards (5+ rated fights) and fighter averages all reflect Verdict MMA fans, 2025 onward. The tab's header says so. Swap the RPC's source subquery back to `ratings` when real users arrive (scraper repo `migrate/community_crowd_ratings.sql`). Tables and view columns were added on September 16, 2026 (scraper repo `migrate/add_crowd_ratings.sql`). Preview event/fighter pages at phone and desktop widths when changing layouts.
